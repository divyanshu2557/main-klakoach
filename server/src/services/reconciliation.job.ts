...
export async function runReconciliation(): Promise<void> {
  const staleOrders = await db.prepare(
    ...
    ).all() as unknown as StalePendingOrder[];
  ...
  for (const order of staleOrders) {
    const payment = (await db.prepare(
          `SELECT id, provider_payment_id, status FROM payments WHERE order_id = ?`
        ).get(order.id)) as unknown as PaymentRow | undefined;
    ...
    try {
      ...
      if (rzpOrder.status === 'paid') {
        ...
        const items = (await db.prepare(`SELECT product_id, quantity FROM order_items WHERE order_id = ?`).all(order.id)) as unknown as any[];
      ...
      await db.prepare(
        `INSERT INTO activity_logs(id, auth_account_id, action, entity, entity_id, metadata, created_at)
         VALUES(?, NULL, 'RECONCILIATION_ERROR', 'orders', ?, ?, datetime('now'))`
      ).run(uuid(), order.id, JSON.stringify({ error: err.message }));
