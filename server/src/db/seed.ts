...
const demoArtisans: DemoArtisan[] = [
  {
...
async function ensureAccount(db: Db, email: string, password: string, kind: "CUSTOMER" | "ARTISAN" | "ADMIN") {
  const existing = (await db.prepare("SELECT id FROM auth_accounts WHERE email = ?").get(email)) as { id: string } | undefined;
...
async function ensureInitialAdmin(db: Db) {
  const adminCount = ((await db.prepare("SELECT COUNT(*) as c FROM auth_accounts WHERE kind = 'ADMIN'").get()) as { c: number })?.c ?? 0;
...
export async function seedDemoData(db: Db) {
  ...
  const authCount = ((await db.prepare("SELECT COUNT(*) as c FROM auth_accounts").get()) as { c: number })?.c ?? 0;
  const productCount = ((await db.prepare("SELECT COUNT(*) as c FROM products").get()) as { c: number })?.c ?? 0;
  const orderCount = ((await db.prepare("SELECT COUNT(*) as c FROM orders").get()) as { c: number })?.c ?? 0;

  const categoryRows = (await db.prepare("SELECT id, slug FROM categories").all()) as { id: string; slug: string }[];
  ...
  if (Number(authCount) === 0) {
      ...
      for (const product of artisan.products) {
        ...
        await db.prepare(
          `INSERT INTO products(id,artisan_id,category_id,title,slug,description,price_cents,status,image_url,featured)
           VALUES(?,?,?,?,?,?,?,?,?,?)`
        ).run(product.id, artisanId, categoryId, product.title, product.slug, product.description, product.price_cents, product.status ?? "ACTIVE", product.image_url, product.featured ?? 0);
    ...
    const customer = (await db.prepare("SELECT id FROM customers WHERE auth_account_id = ?").get(customerId)) as { id: string } | undefined;
    const seedProducts = (await db.prepare("SELECT id, price_cents FROM products ORDER BY created_at ASC LIMIT 2").all()) as { id: string; price_cents: number }[];
    if (customer && seedProducts && seedProducts.length > 0) {
      ...
      for (const artisan of demoArtisans.slice(0, 2)) {
        const artisanRow = (await db.prepare("SELECT id FROM artisans WHERE studio_name = ?").get(artisan.studioName)) as { id: string } | undefined;
        if (artisanRow) {
          try {
  ...
  if (Number(productCount) === 0 && Number(authCount) > 0) {
    const artisans = (await db.prepare("SELECT id, studio_name FROM artisans").all()) as { id: string; studio_name: string }[];
    if (artisans && artisans.length > 0) {
      ...
      if (fallbackCategory) {
        ...
        await db.prepare(
          `INSERT INTO products(id,artisan_id,category_id,title,slug,description,price_cents,status,image_url,featured)
           VALUES(?,?,?,?,?,?,?,?,?,1)`
        ).run(sampleProductId, artisans[0].id, fallbackCategory, "Signature Atelier Piece", "signature-atelier-piece", "Living catalog entry for the public storefront.", 199000, "ACTIVE", "https://images.unsplash.com/photo-1493106641515-6b5631de4bb9?auto=format&fit=crop&w=1200&q=85");
        await db.prepare("INSERT INTO inventory(id,product_id,quantity,reserved,low_stock_at) VALUES(?,?,?,?,?)")
  ...
  if (Number(orderCount) === 0 && Number(authCount) > 0) {
    const customer = (await db.prepare("SELECT id FROM customers LIMIT 1").get()) as { id: string } | undefined;
    const sampleProduct = (await db.prepare("SELECT id, price_cents FROM products ORDER BY created_at ASC LIMIT 1").get()) as { id: string; price_cents: number } | undefined;
