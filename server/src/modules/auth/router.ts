...
async function logActivity(authAccountId: string | null, action: string, entity: string, entityId?: string, ip?: string) {
  await db.prepare(
    "INSERT INTO activity_logs(id,auth_account_id,action,entity,entity_id,ip_address) VALUES(?,?,?,?,?,?)"
  ).run(uuid(), authAccountId, action, entity, entityId ?? null, ip ?? null);
}

// Helper: check if registration is enabled via site_settings
async function isRegistrationEnabled(): Promise<boolean> {
  const row = await db.prepare("SELECT value FROM site_settings WHERE key = 'registration_enabled'").get() as unknown as { value: string } | undefined;
...
authRouter.post("/register", async (req, res) => {
  ...
  const tx = db.transaction(async () => {
    await db.prepare(
      "INSERT INTO auth_accounts(id,email,password_hash,kind) VALUES(?,?,?,?)"
    ).run(accountId, email, passwordHash, kind);

    if (kind === "CUSTOMER") {
      await db.prepare("INSERT INTO customers(id,auth_account_id,name) VALUES(?,?,?)").run(profileId, accountId, name);
    } else {
      await db.prepare(
        "INSERT INTO artisans(id,auth_account_id,studio_name,story,approved) VALUES(?,?,?,?,0)"
      ).run(profileId, accountId, studioName ?? name, "");
    }
  });
  await tx();
...
authRouter.post("/login", async (req, res) => {
  ...
  const maxAttemptsRow = (await db.prepare("SELECT value FROM site_settings WHERE key = 'max_login_attempts'").get()) as unknown as { value: string } | undefined;
  ...
  const failedCount = (await db.prepare(
     ...
     WHERE action = 'LOGIN_FAILED' AND entity = 'auth_accounts'
       AND ip_address = ? AND created_at >= datetime('now', '-15 minutes')`
    ).get(req.ip ?? "") as unknown as { c: number }).c;
  ...
  const account = (await db.prepare(
      "SELECT id, password_hash, kind, suspended FROM auth_accounts WHERE email = ?"
    ).get(email)) as unknown as { id: string; password_hash: string; kind: string; suspended: number } | undefined;
  ...
  if (account.kind === "ARTISAN") {
    const a = (await db.prepare("SELECT id FROM artisans WHERE auth_account_id = ?").get(account.id)) as unknown as { id: string } | undefined;
    artisanId = a?.id;
  }
  ...
  await db.prepare(
    "INSERT INTO sessions(id,auth_account_id,refresh_hash,expires_at,ip_address) VALUES(?,?,?,?,?)"
  ).run(sessionId, account.id, refreshHash, expiresAt, req.ip ?? "");
...
authRouter.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    const session = await db.prepare(
          "SELECT * FROM sessions WHERE id = ? AND auth_account_id = ? AND expires_at > datetime('now')"
        ).get(payload.sessionId, payload.sub) as unknown as { id: string; auth_account_id: string; refresh_hash: string } | undefined;
    ...
    const account = (await db.prepare("SELECT id, kind, suspended FROM auth_accounts WHERE id = ?").get(payload.sub)) as unknown as { id: string; kind: string; suspended: number };
    if (account.suspended) {
...
authRouter.get("/me", authenticate, async (req, res) => {
  const user = (req as AuthRequest).user;
  const account = (await db.prepare("SELECT id, email, kind, created_at FROM auth_accounts WHERE id = ?").get(user.sub)) as Record<string, unknown>;
  if (!account) {
