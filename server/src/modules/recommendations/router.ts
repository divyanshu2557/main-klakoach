...
import type { JwtPayload } from "../../security/tokens.js";
...
function tryGetCustomer(authHeader: string | undefined): {
  customerId: string;
  ...
  const customer = db
      ...
      .get(payload.sub) as unknown as { id: string } | undefined;
...
recommendationsRouter.get("/", async (req, res) => {
  ...
  if (productId) {
    const target = db
          .prepare(
            `SELECT p.id, p.category_id, p.price_cents
         FROM products p
         WHERE p.id = ? AND p.status = 'ACTIVE'`
          )
          .get(productId) as unknown as { id: string; category_id: string; price_cents: number } | undefined;
    ...
    const similar = db
         ...
         LIMIT 8`
          ...
          .all(target.category_id, target.id, target.price_cents, priceTolerance) as unknown as ProductRow[];
  ...
  if (auth) {
    ...
    const eventCount = (
      db
                ...
                .get(auth.customerId) as unknown as { cnt: number }
    ).cnt;
    ...
    if (eventCount >= 5) {
      ...
      const browseEvents = db
              .prepare(
                `SELECT be.product_id, be.event_type, be.created_at, p.category_id
           ...
           LIMIT 500`
              ...
              .all(auth.customerId) as unknown as {
      ...
      for (const ev of browseEvents) {
        if (
          (ev.event_type === "VIEW" || ev.event_type === "CART_ADD") &&
          ev.category_id
        ) {
      ...
      const wishlistRows = db
              ...
              .all(auth.customerId) as unknown as { product_id: string }[];
