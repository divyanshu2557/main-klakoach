...
adminRouter.get("/analytics", async (_req, res) => {
  const gmv = await db.prepare(
      `SELECT COALESCE(SUM(total_cents), 0) as total FROM orders WHERE status IN ('PAID','FULFILLING','SHIPPED','DELIVERED')`
  ...
  const topArtisans = await db.prepare(
    `SELECT a.studio_name, COUNT(DISTINCT p.id) as products,
  ...
  const topProducts = await db.prepare(
    `SELECT p.title, p.price_cents, COALESCE(SUM(oi.quantity), 0) as units_sold
  ...
  const fraudAlerts = await db.prepare(
    `SELECT o.id, o.fraud_score, o.total_cents, o.status, o.created_at, cu.name as customer
     FROM orders o JOIN customers cu ON cu.id = o.customer_id
     WHERE o.fraud_score > 0.3 ORDER BY o.fraud_score DESC LIMIT 20`
  ).all();

  const recentActivity = await db.prepare(
    `SELECT al.action, al.entity, al.created_at, aa.email
     FROM activity_logs al LEFT JOIN auth_accounts aa ON aa.id = al.auth_account_id
     ORDER BY al.created_at DESC LIMIT 50`
  ).all();

  const dailyRevenue = await db.prepare(
    `SELECT DATE(created_at) as date, SUM(total_cents) as revenue, COUNT(*) as orders
...
adminRouter.get("/catalog/products", async (_req, res) => {
  const products = await db.prepare(
    `SELECT p.id, p.title, p.slug, p.description, p.price_cents, p.status, p.featured,
...
adminRouter.post("/catalog/products", requirePermission("inventory:write"), async (req, res) => {
  ...
  const artisan = (await db.prepare("SELECT id FROM artisans WHERE id = ?").get(parsed.data.artisanId)) as unknown as { id: string } | undefined;
  const category = (await db.prepare("SELECT id FROM categories WHERE id = ?").get(parsed.data.categoryId)) as unknown as { id: string } | undefined;
  if (!artisan || !category) {
  ...
  await db.prepare(
    `INSERT INTO products(id,artisan_id,category_id,title,slug,description,price_cents,status,image_url,featured)
     VALUES(?,?,?,?,?,?,?,?,?,?)`
  ).run(id, artisan.id, category.id, parsed.data.title, slug, parsed.data.description, parsed.data.priceCents, parsed.data.status, parsed.data.imageUrl, parsed.data.featured ? 1 : 0);
...
adminRouter.patch("/catalog/products/:id", requirePermission("inventory:write"), async (req, res) => {
  ...
  const product = (await db.prepare("SELECT id FROM products WHERE id = ?").get(req.params.id)) as unknown as { id: string } | undefined;
  if (!product) {
  ...
  if (parsed.data.title) await db.prepare("UPDATE products SET title=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.title, req.params.id);
  if (parsed.data.description !== undefined) await db.prepare("UPDATE products SET description=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.description, req.params.id);
  if (parsed.data.priceCents !== undefined) await db.prepare("UPDATE products SET price_cents=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.priceCents, req.params.id);
  if (parsed.data.imageUrl !== undefined) await db.prepare("UPDATE products SET image_url=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.imageUrl, req.params.id);
  if (parsed.data.status !== undefined) await db.prepare("UPDATE products SET status=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.status, req.params.id);
  if (parsed.data.featured !== undefined) await db.prepare("UPDATE products SET featured=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.featured ? 1 : 0, req.params.id);
  if (parsed.data.stock !== undefined) await db.prepare("UPDATE inventory SET quantity=?,updated_at=datetime('now') WHERE product_id=?").run(parsed.data.stock, req.params.id);
...
adminRouter.get("/catalog/artisans", async (_req, res) => {
  const artisans = await db.prepare(
    `SELECT a.id, a.studio_name, a.story, a.approved, a.featured, a.created_at,
            aa.email,
...
adminRouter.patch("/catalog/artisans/:id", requirePermission("users:write"), async (req, res) => {
  ...
  const artisan = (await db.prepare("SELECT id FROM artisans WHERE id = ?").get(req.params.id)) as unknown as { id: string } | undefined;
  if (!artisan) {
    res.status(404).json({ error: "NOT_FOUND" });
    return;
  }

  if (parsed.data.studioName !== undefined) await db.prepare("UPDATE artisans SET studio_name=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.studioName, req.params.id);
  if (parsed.data.story !== undefined) await db.prepare("UPDATE artisans SET story=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.story, req.params.id);
  if (parsed.data.approved !== undefined) await db.prepare("UPDATE artisans SET approved=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.approved ? 1 : 0, req.params.id);
  if (parsed.data.featured !== undefined) await db.prepare("UPDATE artisans SET featured=?,updated_at=datetime('now') WHERE id=?").run(parsed.data.featured ? 1 : 0, req.params.id);
...
adminRouter.patch("/catalog/categories/:id", requirePermission("users:write"), async (req, res) => {
  ...
  const category = (await db.prepare("SELECT id FROM categories WHERE id = ?").get(req.params.id)) as unknown as { id: string } | undefined;
  if (!category) {
    res.status(404).json({ error: "NOT_FOUND" });
    return;
  }

  if (parsed.data.name !== undefined) await db.prepare("UPDATE categories SET name=? WHERE id=?").run(parsed.data.name, req.params.id);
  if (parsed.data.slug !== undefined) await db.prepare("UPDATE categories SET slug=? WHERE id=?").run(slugify(parsed.data.slug), req.params.id);
  if (parsed.data.sortOrder !== undefined) await db.prepare("UPDATE categories SET sort_order=? WHERE id=?").run(parsed.data.sortOrder, req.params.id);
...
adminRouter.get("/users", requirePermission("users:write"), async (req, res) => {
  ...
  const total = (await db.prepare(`SELECT COUNT(*) as c FROM auth_accounts ${where}`).get(...params) as unknown as { c: number }).c;
  const users = await db.prepare(
    `SELECT id, email, kind, suspended, created_at FROM auth_accounts ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`
...
adminRouter.patch("/users/:id/status", requirePermission("users:write"), async (req, res) => {
  ...
  if (!["suspend", "activate"].includes(action)) { res.status(400).json({ error: "INVALID_ACTION" }); return; }
  ...
  await db.prepare(
    "INSERT INTO activity_logs(id,auth_account_id,action,entity,entity_id,ip_address) VALUES(?,?,?,?,?,?)"
  ).run(uuid(), user.sub, `ADMIN_${action.toUpperCase()}`, "auth_accounts", req.params.id, req.ip ?? "");
...
adminRouter.get("/artisans/pending", async (_req, res) => {
  const pending = await db.prepare(
    `SELECT a.id, a.studio_name, a.story, a.created_at, aa.email
     FROM artisans a JOIN auth_accounts aa ON aa.id = a.auth_account_id
     WHERE a.approved = 0 ORDER BY a.created_at ASC`
  ).all();
  res.json(pending);
});
...
adminRouter.patch("/artisans/:id/approve", requirePermission("users:write"), async (req, res) => {
  ...
  const artisan = (await db.prepare("SELECT auth_account_id FROM artisans WHERE id = ?").get(req.params.id)) as unknown as { auth_account_id: string } | undefined;
  if (artisan) {
    await db.prepare("INSERT INTO notifications(id,auth_account_id,title,body) VALUES(?,?,?,?)").run(
      uuid(), artisan.auth_account_id,
      approved ? "Studio Approved!" : "Application Update",
      approved ? "Your artisan studio has been approved. You can now list products." : "Your application requires additional review."
    );
  }
...
adminRouter.get("/products/review", async (_req, res) => {
  const products = await db.prepare(
    `SELECT p.id, p.title, p.price_cents, p.status, p.created_at, a.studio_name as artisan
...
adminRouter.get("/orders", requirePermission("orders:read"), async (req, res) => {
  ...
  if (highFraud) { where += " AND o.fraud_score > 0.3"; }

  const total = (await db.prepare(`SELECT COUNT(*) as c FROM orders o ${where}`).get(...params) as unknown as { c: number }).c;
...
adminRouter.get("/settings", requirePermission("admin:settings"), async (_req, res) => {
  const rows = (await db.prepare("SELECT key, value, updated_at FROM site_settings ORDER BY key").all()) as unknown as { key: string; value: string; updated_at: string }[];
...
adminRouter.patch("/settings", requirePermission("admin:settings"), async (req, res) => {
  ...
  for (const [key, value] of entries) {
    await db.prepare(
      "INSERT INTO activity_logs(id,auth_account_id,action,entity,entity_id,metadata,ip_address) VALUES(?,?,?,?,?,?,?)"
    ).run(uuid(), user.sub, "SETTINGS_CHANGED", "site_settings", key, JSON.stringify({ key, value }), req.ip ?? "");
  }
...
adminRouter.get("/security/sessions", requirePermission("security:read"), async (_req, res) => {
  const sessions = await db.prepare(
    `SELECT s.id, s.auth_account_id, s.created_at, s.expires_at, s.ip_address,
            aa.email, aa.kind
     FROM sessions s
...
adminRouter.delete("/security/sessions/:id", requirePermission("security:read"), async (req, res) => {
  const user = (req as AuthRequest).user;
  const session = (await db.prepare("SELECT id, auth_account_id FROM sessions WHERE id = ?").get(req.params.id)) as unknown as { id: string; auth_account_id: string } | undefined;
  if (!session) { res.status(404).json({ error: "NOT_FOUND" }); return; }

  await db.prepare("DELETE FROM sessions WHERE id = ?").run(req.params.id);
  await db.prepare(
    "INSERT INTO activity_logs(id,auth_account_id,action,entity,entity_id,ip_address) VALUES(?,?,?,?,?,?)"
  ).run(uuid(), user.sub, "ADMIN_SESSION_KILLED", "sessions", req.params.id, req.ip ?? "");
...
adminRouter.get("/security/events", requirePermission("security:read"), async (_req, res) => {
  const events = await db.prepare(
    `SELECT al.id, al.action, al.entity, al.entity_id, al.ip_address, al.created_at, al.metadata,
            aa.email
...
adminRouter.get("/financials", requirePermission("analytics:read"), async (req, res) => {
  ...
  const gmvRow = await db.prepare(
      `SELECT COALESCE(SUM(total_cents), 0) as gmv, COUNT(*) as transaction_count
     FROM orders
     WHERE status IN ('PAID','FULFILLING','SHIPPED','DELIVERED')
       AND created_at BETWEEN ? AND ?`
    ).get(from, to) as unknown as { gmv: number; transaction_count: number };

  const gmvCents = gmvRow.gmv;
  const transactionCount = gmvRow.transaction_count;

  // Refund total
  const refundsRow = await db.prepare(
      `SELECT COALESCE(SUM(total_cents), 0) as refunds
     FROM orders
     WHERE status = 'REFUNDED'
       AND created_at BETWEEN ? AND ?`
    ).get(from, to) as unknown as { refunds: number };
  ...
  const payoutsRow = await db.prepare(
      `SELECT COALESCE(SUM(amount_cents), 0) as payouts FROM artisan_payouts WHERE status = 'PAID'`
    ).get() as unknown as { payouts: number };
  ...
  const perArtisan = await db.prepare(
    `SELECT
...
adminRouter.get("/orders/:id", requirePermission("orders:read"), async (req, res) => {
  const order = (await db.prepare(
    `SELECT o.id, o.status, o.total_cents, o.fraud_score, o.created_at, o.updated_at,
  ...
  const items = await db.prepare(
    `SELECT p.title, oi.quantity, oi.price_cents, p.image_url
     FROM order_items oi
  ...
  const payment = (await db.prepare(
    `SELECT provider, status, amount_cents FROM payments WHERE order_id = ?`
  ).get(req.params.id)) as Record<string, unknown> | null;

  const shipping = (await db.prepare(
    `SELECT carrier, tracking_number as trackingNumber, status FROM shipping WHERE order_id = ?`
  ).get(req.params.id)) as Record<string, unknown> | null;
...
adminRouter.patch("/orders/bulk", requirePermission("orders:write"), async (req, res) => {
  const { orders } = req.body as { orders?: Array<{ id: string; status: string; trackingNumber?: string; carrier?: string }> };
  ...
  for (const item of orders) {
    try {
      const existing = (await db.prepare("SELECT id, status FROM orders WHERE id = ?").get(item.id)) as unknown as { id: string; status: string } | undefined;
      ...
      if (item.status === "SHIPPED") {
        ...
        await db.prepare(
          `INSERT OR REPLACE INTO shipping(id, order_id, carrier, tracking_number, status, updated_at)
           VALUES(COALESCE((SELECT id FROM shipping WHERE order_id = ?), ?), ?, ?, ?, 'SHIPPED', datetime('now'))`
        ).run(item.id, uuid(), item.id, item.carrier, item.trackingNumber);
      }
...
adminRouter.get("/security/inactive-admins", requirePermission("security:read"), async (_req, res) => {
  const inactiveAdmins = await db.prepare(
    `SELECT aa.id, aa.email, aa.created_at
...
adminRouter.patch("/payouts/:artisanId", requirePermission("analytics:read"), async (req, res) => {
  ...
  await db.prepare(
    `INSERT INTO activity_logs(id, auth_account_id, action, entity, entity_id, ip_address)
     VALUES(?, ?, 'PAYOUT_INITIATED', 'artisan_payouts', ?, ?)`
  ).run(uuid(), user.sub, req.params.artisanId, req.ip ?? "");
