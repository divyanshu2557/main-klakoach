...
customerRouter.get(
  ...
  async (req, res) => {
    ...
    const profile = await db
          .prepare(
            `SELECT c.id, c.name, a.email
         FROM customers c
         JOIN auth_accounts a ON a.id = c.auth_account_id
         WHERE c.auth_account_id = ?`
          )
          .get(user.sub) as unknown as { id: string; name: string; email: string } | undefined;
    ...
    const addresses = await db
      .prepare(
        `SELECT id, line1, city, country, postal, created_at
    ...
    const orderCountRow = await db
          ...
          .get(profile.id) as unknown as { c: number };
    ...
    const spendRow = await db
          ...
          .get(profile.id) as unknown as { s: number };
...
customerRouter.patch(
  ...
  async (req, res) => {
    ...
    const result = await db
      .prepare(
        "UPDATE customers SET name = ?, updated_at = datetime('now') WHERE auth_account_id = ?"
      )
...
customerRouter.post(
  ...
  async (req, res) => {
    const user = (req as AuthRequest).user;
    const { line1, city, country, postal } = req.body as Record<string, unknown>;
    ...
    const customer = await db
          ...
          .get(user.sub) as unknown as { id: string } | undefined;
    ...
    const id = uuid();
    await db.prepare(
      `INSERT INTO addresses(id, customer_id, line1, city, country, postal, created_at)
       VALUES(?, ?, ?, ?, ?, ?, datetime('now'))`
    ).run(id, customer.id, line1.trim(), city.trim(), country.trim(), postal.trim());
