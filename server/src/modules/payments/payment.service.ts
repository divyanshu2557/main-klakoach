...
interface CheckoutIntentResult {
  clientSecret: string; // Will store razorpay order ID
  orderId: string;
  amountCents: number;
}
...
export async function createCheckoutIntent(
  ...
  for (const item of items) {
    const product = await db
          ...
          .get(item.productId) as unknown as ProductRow | undefined;

    if (!product || product.status !== 'ACTIVE') {
      throw Object.assign(
        new Error(`Product ${item.productId} is not available`),
    ...
    const inv = await db
          ...
          .get(item.productId) as unknown as InventoryRow | undefined;
  ...
  const insertOrderAndItems = db.transaction(async () => {
    // Insert order
    await db.prepare(
      `INSERT INTO orders (id, customer_id, status, total_cents, fraud_score, coupon_code, discount_cents, guest_email)
       VALUES (?, ?, 'PENDING', ?, 0, ?, ?, ?)`
    ).run(
      orderId,
      customerId,
  ...
  // Insert payment row
  await db.prepare(
    `INSERT INTO payments (id, order_id, provider, provider_payment_id, status, amount_cents)
     VALUES (?, ?, 'razorpay', ?, 'REQUIRES_ACTION', ?)`
  ).run(uuid(), orderId, rzpOrder.id, chargeAmount);
...
export async function refundOrder(
  ...
  if (order.status !== 'PAID' && order.status !== 'DELIVERED') {
    throw Object.assign(
      new Error(`Order ${orderId} is not eligible for refund (status: ${order.status})`),
  ...
  await db.prepare(
    `INSERT INTO activity_logs (id, auth_account_id, action, entity, entity_id, created_at)
     VALUES (?, ?, 'ORDER_REFUNDED', 'orders', ?, datetime('now'))`
  ).run(uuid(), adminAccountId, orderId);

  // Notify the customer
  const customer = await db
      .prepare(`SELECT auth_account_id FROM customers WHERE id = ?`)
      .get(order.customer_id) as unknown as CustomerAuthRow | undefined;
...
export async function handleWebhookEvent(event: any): Promise<void> {
  switch (event.event) {
    case 'payment.captured': {
      ...
      if (orderId) await _handlePaymentSucceeded(orderId);
      break;
    }

    case 'payment.failed': {
      const orderId = event.payload.payment.entity.order_id;
      if (orderId) await _handlePaymentFailed(orderId);
      break;
    }
    ...
    case 'refund.processed': {
      ...
      if (paymentId) await _handleChargeRefunded(paymentId);
      break;
    }
...
async function _handlePaymentSucceeded(razorpayOrderId: string): Promise<void> {
  // Idempotency check
  const payment = await db
      .prepare(`SELECT id, order_id, status FROM payments WHERE provider_payment_id = ?`)
      .get(razorpayOrderId) as unknown as (PaymentRow & { order_id: string }) | undefined;
  ...
  const artisanAuthAccounts = await db
      .prepare(
        `SELECT DISTINCT ar.auth_account_id
       FROM order_items oi
       JOIN products p ON p.id = oi.product_id
       JOIN artisans ar ON ar.id = p.artisan_id
       WHERE oi.order_id = ?`
      )
      .all(orderId) as unknown as ArtisanAuthRow[];
...
async function _handlePaymentFailed(razorpayOrderId: string): Promise<void> {
  const payment = await db
      .prepare(`SELECT id, order_id, status FROM payments WHERE provider_payment_id = ?`)
      .get(razorpayOrderId) as unknown as (PaymentRow & { order_id: string }) | undefined;
...
async function _handleChargeRefunded(paymentId: string): Promise<void> {
