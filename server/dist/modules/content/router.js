...
contentRouter.get("/home", async (_req, res) => {
    const products = await db.prepare(`SELECT p.id, p.title, p.slug, p.price_cents, p.image_url, p.description,
    ...
    const artisans = await db.prepare(`SELECT a.id, a.studio_name, a.story, a.featured,
     ...
     LIMIT 8`).all();
    const categories = await db.prepare(`SELECT c.id, c.name, c.slug, c.sort_order,
            COUNT(p.id) as product_count,
            COALESCE(MAX(p.image_url), '') as image_url
     FROM categories c
     LEFT JOIN products p ON p.category_id = c.id AND p.status = 'ACTIVE'
     GROUP BY c.id
     ORDER BY c.sort_order ASC, product_count DESC, c.name ASC`).all();
    const testimonials = (await db.prepare(`SELECT r.body, r.rating, r.created_at, cu.name as customer_name, p.title as product_title
     FROM reviews r
    ...
    const rawStats = {
        products: (await db.prepare("SELECT COUNT(*) as c FROM products WHERE status = 'ACTIVE'").get()).c,
        artisans: (await db.prepare("SELECT COUNT(*) as c FROM artisans WHERE approved = 1").get()).c,
