...
artisanRouter.get("/profile", authenticate, requirePermission("artisan:profile"), async (req, res) => {
    const user = req.user;
    const artisan = await db.prepare(`SELECT a.id, a.studio_name, a.story, a.approved, a.created_at,
            COUNT(DISTINCT p.id) as product_count
     FROM artisans a
     LEFT JOIN products p ON p.artisan_id = a.id AND p.status = 'ACTIVE'
     WHERE a.auth_account_id = ?
     GROUP BY a.id`).get(user.sub);
    if (!artisan) {
...
artisanRouter.get("/analytics", authenticate, requirePermission("artisan:profile"), async (req, res) => {
    const user = req.user;
    const artisan = (await db.prepare("SELECT id FROM artisans WHERE auth_account_id = ?").get(user.sub));
    if (!artisan) {
        res.status(404).json({ error: "NOT_FOUND" });
        return;
    }
    const totalRevenue = await db.prepare(`SELECT COALESCE(SUM(oi.price_cents * oi.quantity), 0) as total
     FROM order_items oi
     JOIN products p ON p.id = oi.product_id
     JOIN orders o ON o.id = oi.order_id
     WHERE p.artisan_id = ? AND o.status IN ('PAID','FULFILLING','SHIPPED','DELIVERED')`).get(artisan.id);
    const topProducts = await db.prepare(`SELECT p.title, p.image_url, p.price_cents,
     ...
     WHERE p.artisan_id = ?
     GROUP BY p.id ORDER BY units_sold DESC LIMIT 5`).all(artisan.id);
    const recentOrders = await db.prepare(`SELECT o.id, o.status, o.total_cents, o.created_at, COUNT(oi.id) as item_count
     FROM orders o
     JOIN order_items oi ON oi.order_id = o.id
     JOIN products p ON p.id = oi.product_id
     WHERE p.artisan_id = ?
     GROUP BY o.id ORDER BY o.created_at DESC LIMIT 10`).all(artisan.id);
    const lowStock = await db.prepare(`SELECT p.title, i.quantity, i.low_stock_at
     FROM inventory i JOIN products p ON p.id = i.product_id
     WHERE p.artisan_id = ? AND i.quantity <= i.low_stock_at AND p.status = 'ACTIVE'`).all(artisan.id);
    const avgRating = await db.prepare(`SELECT COALESCE(AVG(r.rating), 0) as avg, COUNT(r.id) as count
     FROM reviews r JOIN products p ON p.id = r.product_id WHERE p.artisan_id = ?`).get(artisan.id);
...
artisanRouter.get("/wishlist", authenticate, requirePermission("wishlist:write"), async (req, res) => {
    ...
    const items = await db.prepare(`SELECT p.id, p.title, p.slug, p.price_cents, p.image_url, a.studio_name as artisan
     FROM wishlists w JOIN products p ON p.id = w.product_id JOIN artisans a ON a.id = p.artisan_id
     WHERE w.customer_id = ? ORDER BY w.created_at DESC`).all(customer.id);
    res.json(items);
});
// Post review
artisanRouter.post("/reviews/:productId", authenticate, requirePermission("wishlist:write"), async (req, res) => {
    ...
    try {
        await db.prepare("INSERT INTO reviews(id,customer_id,product_id,rating,body) VALUES(?,?,?,?,?)").run(uuid(), customer.id, req.params.productId, rating, body);
...
artisanRouter.get("/earnings", authenticate, requirePermission("artisan:profile"), async (req, res) => {
    const user = req.user;
    const artisan = (await db.prepare("SELECT id, approved FROM artisans WHERE auth_account_id = ?").get(user.sub));
    if (!artisan) {
        res.status(404).json({ error: "NOT_FOUND" });
        return;
    }
    ...
    const lifetimeRow = await db.prepare(`SELECT COALESCE(SUM(oi.price_cents * oi.quantity), 0) as total
     FROM order_items oi
     JOIN products p ON p.id = oi.product_id
     JOIN orders o ON o.id = oi.order_id
     WHERE p.artisan_id = ?
       AND o.status IN ('PAID','FULFILLING','SHIPPED','DELIVERED')`).get(artisan.id);
    const perProduct = await db.prepare(`SELECT p.id as productId, p.title,
     ...
     ORDER BY revenueCents DESC`).all(artisan.id);
    const dailySeries = await db.prepare(`SELECT DATE(o.created_at) as date,
            COALESCE(SUM(oi.price_cents * oi.quantity), 0) as revenueCents,
     ...
     ORDER BY date ASC`).all(artisan.id);
    res.json({
...
artisanRouter.get("/payouts", authenticate, requirePermission("artisan:profile"), async (req, res) => {
    ...
    const payouts = await db.prepare(`SELECT id, amount_cents as amountCents, status, period, created_at as createdAt
     FROM artisan_payouts
     WHERE artisan_id = ?
     ORDER BY created_at DESC`).all(artisan.id);
    res.json(payouts);
});
// Get notifications
artisanRouter.get("/notifications", authenticate, async (req, res) => {
    const user = req.user;
    const notifs = await db.prepare("SELECT * FROM notifications WHERE auth_account_id = ? ORDER BY created_at DESC LIMIT 20").all(user.sub);
    res.json(notifs);
});
...
artisanRouter.post("/products/:id/media", authenticate, requirePermission("media:write"), async (req, res, next) => {
    ...
    if (user.role !== "ADMIN") {
        ...
        const product = (await db.prepare("SELECT artisan_id FROM products WHERE id = ?").get(productId));
        if (!product) {
            res.status(404).json({ error: "NOT_FOUND" });
            return;
        }
    ...
    const countRow = await db.prepare("SELECT COUNT(*) as cnt FROM product_media WHERE product_id = ?").get(productId);
    if (countRow.cnt >= 8) {
    ...
    const alt = req.body?.alt || req.file.originalname || fileId;
    // Sort order: one past current max
    const sortRow = await db.prepare("SELECT COALESCE(MAX(sort_order), -1) as maxSort FROM product_media WHERE product_id = ?").get(productId);
    const sortOrder = sortRow.maxSort + 1;
    // Use forward-slash URL for serving compatibility
    const url = `uploads/${productId}/${fileId}.${ext}`;
    const mediaId = uuid();
    await db.prepare("INSERT INTO product_media(id, product_id, url, alt, sort_order) VALUES(?, ?, ?, ?, ?)").run(mediaId, productId, url, alt, sortOrder);
    const mediaRow = await db.prepare("SELECT * FROM product_media WHERE id = ?").get(mediaId);
    res.status(201).json(mediaRow);
});
// ── Product Media Delete ──────────────────────────────────────────────────────
artisanRouter.delete("/products/:id/media/:mediaId", authenticate, requirePermission("media:write"), async (req, res) => {
    const user = req.user;
    const productId = req.params.id;
