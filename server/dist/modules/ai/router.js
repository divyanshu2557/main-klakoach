...
aiRouter.get("/recommendations", async (req, res) => {
    const { productId, context = "marketplace" } = req.query;
    ...
    try {
        ...
        const limit = { homepage: 4, cart: 3, checkout: 2, marketplace: 4 };
...
searchKeywords: 3-5 terms useful for finding similar handmade products.`
        ...
        let detected;
        try {
            detected = JSON.parse(result.choices[0].message.content);
        }
        catch {
...
aiRouter.post("/gift-finder", async (req, res) => {
    const { occasion, recipient, budgetMin = 500, budgetMax = 10000, interests = "" } = req.body;
    const products = await db.prepare(`
    SELECT p.id, p.title, p.price_cents, p.description, p.image_url,
  ...
  `).all(budgetMin * 100, budgetMax * 100);
    if (!products.length) {
        res.json({ gifts: [] });
        return;
    }
...
aiRouter.post("/generate-listing", authenticate, requirePermission("inventory:write"), async (req, res) => {
    const { imageUrl, category, material } = req.body;
    if (!imageUrl) {
        res.status(400).json({ error: "IMAGE_URL_REQUIRED" });
        return;
    }
    try {
        const result = await openai.chat.completions.create({
...
aiRouter.post("/translate", authenticate, requirePermission("inventory:write"), async (req, res) => {
    const { productId, targetLanguage } = req.body;
...
Original: ${JSON.stringify({ title: product.title, description: product.description })}`
        ...
        let translated;
        try {
            translated = JSON.parse(result.choices[0].message.content);
        }
        catch {
...
aiRouter.post("/pricing-assist", authenticate, requirePermission("inventory:write"), async (req, res) => {
    const { title, category, description, material } = req.body;
...
aiRouter.post("/fraud-analyze", authenticate, requirePermission("analytics:read"), async (req, res) => {
  ...
  `).get(orderId);
    ...
    const history = await db.prepare("SELECT COUNT(*) as c FROM orders o JOIN customers cu ON cu.id = o.customer_id WHERE cu.name = ?").get(order.customer_name);
    try {
        const result = await openai.chat.completions.create({
            model: CHAT_MODEL,
            ...DEFAULT_AI_PARAMS,
...
aiRouter.post("/quality-check", authenticate, requirePermission("analytics:read"), async (req, res) => {
    const { productId, imageUrl, title, description } = req.body;
    const duplicates = await db.prepare("SELECT COUNT(*) as c FROM products WHERE title LIKE ? AND id != ? AND status != 'ARCHIVED'").get(`%${title.substring(0, 20)}%`, productId);
    try {
        const result = await openai.chat.completions.create({
            model: CHAT_MODEL,
            ...DEFAULT_AI_PARAMS,
...
aiRouter.get("/admin-insights", authenticate, requirePermission("analytics:read"), async (req, res) => {
    const gmv = (await db.prepare("SELECT COALESCE(SUM(total_cents),0) as t FROM orders WHERE status IN ('PAID','FULFILLING','SHIPPED','DELIVERED')").get()).t;
    const orderCount = (await db.prepare("SELECT COUNT(*) as c FROM orders").get()).c;
    const activeProducts = (await db.prepare("SELECT COUNT(*) as c FROM products WHERE status='ACTIVE'").get()).c;
    const pendingProducts = (await db.prepare("SELECT COUNT(*) as c FROM products WHERE status='PENDING_REVIEW'").get()).c;
    const pendingArtisans = (await db.prepare("SELECT COUNT(*) as c FROM artisans WHERE approved=0").get()).c;
    const fraudAlerts = (await db.prepare("SELECT COUNT(*) as c FROM orders WHERE fraud_score > 0.3").get()).c;
    const lowStock = (await db.prepare("SELECT COUNT(*) as c FROM inventory WHERE quantity <= low_stock_at AND quantity > 0").get()).c;
    try {
        const result = await openai.chat.completions.create({
            model: CHAT_MODEL,
            ...DEFAULT_AI_PARAMS,
...
aiRouter.post("/artisan-story", authenticate, requirePermission("artisan:profile"), async (req, res) => {
    const user = req.user;
    const { studioName, craft, story, location, yearsActive } = req.body;
    try {
        const result = await openai.chat.completions.create({
            model: CHAT_MODEL,
            ...DEFAULT_AI_PARAMS,
