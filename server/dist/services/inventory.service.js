    ...
    const tx = db.transaction(async () => {
        for (const item of items) {
            ...
            if (available < item.quantity) {
                throw new Error(`INSUFFICIENT_STOCK:${item.productId}:available=${available}:requested=${item.quantity}`);
            }
...
export async function releaseInventory(orderId) {
    ...
    const tx = db.transaction(async () => {
        for (const item of items) {
            await db.prepare(`UPDATE inventory SET reserved = MAX(0, reserved - ?), updated_at = datetime('now') WHERE product_id = ?`).run(item.quantity, item.product_id);
        }
    });
    await tx();
}
 ...
 */
export async function checkAndNotifyLowStock(productId) {
    const inv = await db
        .prepare(`SELECT quantity, reserved, low_stock_at FROM inventory WHERE product_id = ?`)
        .get(productId);
    if (!inv)
        return;
    const available = inv.quantity - inv.reserved;
    if (available > inv.low_stock_at)
        return;
    // Check if a LOW_STOCK_NOTIFIED entry exists within the last 24 hours
    const recentNotification = await db
        .prepare(`SELECT id FROM activity_logs
    ...
    const productInfo = await db
        .prepare(`SELECT p.title, a.auth_account_id
       FROM products p
    ...
    try {
        ...
        const mod = await (new Function('p', 'return import(p)'))(notifPath);
    ...
    await db.prepare(`INSERT INTO activity_logs(id, action, entity, entity_id, metadata, created_at)
     VALUES(?, 'LOW_STOCK_NOTIFIED', 'products', ?, ?, datetime('now'))`).run(uuid(), productId, JSON.stringify({ available, low_stock_at: inv.low_stock_at }));
}
