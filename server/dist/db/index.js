...
function translateSqliteQuery(query) {
    let counter = 1;
    let pgQuery = query.replace(/\?/g, () => `$${counter++}`);
    pgQuery = pgQuery.replace(/datetime\('now',\s*'([+-])(\d+)\s+(days|hours|minutes|seconds)'\)/g, (_match, sign, amount, unit) => `NOW() ${sign === "-" ? "-" : "+"} INTERVAL '${amount} ${unit}'`);
...
export const db = {
    prepare: (query) => {
        ...
        return {
            all: async (...args) => await sql.query(pgQuery, args),
            get: async (...args) => (await sql.query(pgQuery, args))[0],
            run: async (...args) => {
...
async function bootstrap() {
  ...
  `);
    // ── Seed defaults ─────────────────────────────────────────────────────────
    const settingsCount = (await db.prepare("SELECT COUNT(*) as c FROM site_settings").get());
